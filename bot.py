...
async def handle_image_with_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    description = description.strip()[:888]
...
async def handle_random_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Шаблон подписи без самого описания
    caption_template = (
        f"🎲 <b>Случайная карта</b>\n\n"
        f"📁 <b>Колода:</b> {escaped_cat_name}\n"
        f"📝 <b>Описание:</b> \n\n"
        f"🔢 <b>Номер:</b> {random_index + 1} из {len(images)}"
    )

    # Вычисляем, сколько чистых символов (включая будущие HTML-сущности) осталось под описание
    # Максимальный лимит Telegram — 1024. Оставляем 1010 с запасом на многоточие "..."
    max_desc_len = 1010 - len(caption_template)

    # Сначала экранируем, чтобы точно знать длину HTML-строки
    escaped_desc = html.escape(description)
    if len(escaped_desc) > max_desc_len:
        # Обрезаем экранированную строку, аккуратно, чтобы не сломать HTML-сущность (например &quot;)
        # Для надежности просто берем срез исходного текста, который гарантированно влезет
        safe_raw_desc = description[:max_desc_len // 6]  # В худшем случае 1 символ превращается в 6 (например, &rarr;)
        escaped_desc = html.escape(safe_raw_desc) + "..."
